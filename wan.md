...
*Добавление модуля HWIC-2T на маршрутизатор 2911 для получения последовательных портов Serial.*
...
*Готовая топология сети согласно схеме*
...
*Настройка интерфейсов f0/0 и f0/1 на R1 с IP-адресами 10.1.1.1/24 и 10.12.12.1/24.*
...
*Настройка интерфейсов f0/0 и f0/1 на R2 с IP-адресами 10.23.23.2/24 и 10.12.12.2/24.*
...
*Настройка тактовой частоты на DCE-порту R3 для работы PPP.*
...
*Настройка CHAP на R3: создание пользователя R1973 и включение PPP CHAP на интерфейсе.*
...
*Настройка CHAP на R1973: создание пользователя R3 и включение PPP CHAP.*

Проверка:

<img width="604" height="446" alt="image" src="https://github.com/user-attachments/assets/543ea156-10e4-4eb9-bff8-340f30040f72" />

*Проверка работы PPP и CHAP — интерфейс serial 0/3/0 в состоянии up/up.*
...
*Настройка router-id 0.0.0.2 на R2 для OSPFv2.*
...
*Настройка OSPF на R1: объявление сетей 10.1.1.0 в зоне 1 и 10.12.12.0 в зоне 0.*

R2:

<img width="501" height="93" alt="image" src="https://github.com/user-attachments/assets/e23e67e8-acbe-4d81-8299-36f287361bc8" />

*Настройка OSPF на R2: объявление сетей 10.12.12.0 в зоне 0 и 10.23.23.0 в зоне 23.*

R3:

<img width="558" height="185" alt="image" src="https://github.com/user-attachments/assets/c076c8f4-1af6-4583-85b0-4417d42083af" />

*Настройка OSPF на R3: объявление сетей 10.23.23.0, 3.3.3.0 и 33.33.33.0 в зоне 23.*
...
*Блокировка отправки hello-пакетов OSPF на всех интерфейсах R1, кроме f0/1.*
...
*Настройка R3 для распространения маршрута по умолчанию в OSPF*
...
*Проверка маршрута по умолчанию на R1.*
...
*Настройка внешнего BGP-соседства между R3 и R1973.*
...
*Объявление loopback-сети 73.73.73.0/24 на R1973 через BGP.*
...
*Настройка маршрута по умолчанию на R1973 в сторону R3.*
...
*Проверка маршрутов, полученных R3 через BGP.*
...
*Проверка маршрута по умолчанию на R1973.*
...
*Проверка поддержки VOIP и расширенных настроек безопасности через оценочную лицензию.*

<img width="555" height="384" alt="image" src="https://github.com/user-attachments/assets/70ea5edf-5f18-4ed2-9b25-ba255cf1352d" />

*Продолжение проверки лицензий и поддержки функций.*
...
*Установка лицензии securityk9 на R3.*
...
*Проверка наличия DHCP-сервера на схеме.*
...
*Настройка IP-адреса на DHCP-сервере.*
...
*Настройка DHCP-ретранслятора на R1.*
...
*Перезапуск OSPF на R2 для корректной работы DHCP.*
...
*Настройка IPv6-адресов на интерфейсах R1.*
...
*Настройка IPv6-адресов на интерфейсах R2.
...
*Настройка IPv6-адресов на интерфейсах R3.*
...
*Настройка IPv6-адресов на интерфейсах R1973.*
...
*Проверка включения IPv6-маршрутизации.*
...
*Проверка link-local адреса fe80::1 на интерфейсе f0/0 R1.*
...
*Проверка использования EUI-64 для глобального адреса на f0/0 R1.*
...
*Блокировка hello-пакетов OSPFv3 на всех интерфейсах R1, кроме f0/1*
...
*Настройка R3 для распространения маршрута IPv6 по умолчанию в OSPFv3*
...
*Проверка соседств OSPFv3 на R1.*
...
*Проверка маршрутов OSPFv3 в таблице IPv6 маршрутизации R1.*
...
*Проверка маршрутов EIGRPv6 на R3.*
...
*Объявление loopback-сети 2001:1973:1973:1973::1973/64 на R1973 через EIGRPv6.*
...
*Настройка маршрута IPv6 по умолчанию на R1973 в сторону R3.*
